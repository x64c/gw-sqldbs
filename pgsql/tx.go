...
func (t *Tx) QueryRow(ctx context.Context, table string, columns []string, id any) sqldbs.Row {
	query := fmt.Sprintf("SELECT %s FROM %s WHERE id = $1 LIMIT 1", sqldbs.QuoteJoinIdentifiers(t.db.client, columns), t.db.client.QuoteIdentifier(table))
	return &Row{row: t.tx.QueryRow(ctx, query, id)}
}
