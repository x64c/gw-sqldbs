...
func loadRawStmtsToStore(store *sqldbs.RawSQLStore, sqlFS fs.FS) error {
	...
	err := fs.WalkDir(sqlFS, ".", func(path string, d fs.DirEntry, err error) error {
		...
		if plainExt != "pgsql" && plainExt != "sql" {
			return nil
		}
		...
		if _, exists := store.Get(key); !exists {
			// Placeholder Conversion: `?` (static) → `$1, $2, ...`
			sql := sqldbs.ReplaceStaticPlaceholders(string(data), '$')
			store.Set(key, sql)
			stmtCnt++
		}
		return nil
	...
	log.Printf("[INFO][pgsql] %d sql raw stmts loaded", stmtCnt)
	return nil
}
